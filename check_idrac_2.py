...
def option_reader():
    ...
    optp.add_option('-w', '--hw', help='hardware to check. If no hardware specified, all will be listed: DISK, VDISK, FAN, SENSOR, CPU, PS, PU, MEM, BATTERY', dest='hardware')
            ...
            if re.search(r'FAN', opts.hardware, re.IGNORECASE):
                if opts.fan_alert:
                    try:
                        ...
                        print 'Missing FAN ALERT threshold.'
                        sys.exit(1)
                ...
                if opts.vdiskbad:
                    try:
                        ...
                        print 'Missing VDISK ALERT threshold.'
                        sys.exit(1)
                ...
                if opts.temp_warn or opts.temp_crit:
                    try:
                        ...
                        print 'Missing TEMP ALERT threshold.'
                        sys.exit(1)
                ...
                if opts.volt_warn or opts.volt_crit:
                    try:
                        ...
                        print 'Missing VOLT ALERT threshold.'
                        sys.exit(1)
                if opts.cur_warn or opts.cur_crit:
                    try:
                        ...
                        print 'Missing CURRENT ALERT threshold.'
                        sys.exit(1)
                if opts.wat_warn or opts.wat_crit:
                    try:
                        ...
                        print 'Missing WATT ALERT threshold.'
                        sys.exit(1)
                ...
                if opts.consume:
                    try:
                        ...
                        print 'Missing PWR ALERT threshold.'
                        sys.exit(1)
...
class PARSER:
                ...
                if type(stat) == int:
                    ...
                    if self.hardware[2] == 'Fan':
                        ...
                        if stat_t == 6:
                            ...
                            if conf['ps_vol_max_crit'] >= tmp[key][stat_t] <= conf['ps_vol_min_crit']:
                                tmp[key][stat_t] = '%.0f(!!)' %tmp[key][stat_t]
                                alert = 2
                            elif conf['ps_vol_max_warn'] >= tmp[key][stat_t] <= conf['ps_vol_min_warn']:
                                tmp[key][stat_t] = '%.0f(!)' %tmp[key][stat_t]
                                if alert != 2:
                                    alert = 1
                            ...
                            if conf['ps_cur_max_crit'] >= tmp[key][stat_t] <= conf['ps_cur_min_crit']:
                                tmp[key][stat_t] = '%.1f(!!)' %tmp[key][stat_t]
                                alert = 2
                            elif conf['ps_cur_max_warn'] >= tmp[key][stat_t] <= conf['ps_cur_min_warn']:
                                tmp[key][stat_t] = '%.1f(!)' %tmp[key][stat_t]
                                if alert != 2:
                                    alert = 1
                            ...
                            tmp[key][stat_t] = float(tmp[key][stat_t])/10
                            if conf['ps_watt_max_crit'] >= tmp[key][stat_t] <= conf['ps_watt_min_crit']:
                                tmp[key][stat_t] = '%.0f(!!)' %tmp[key][stat_t]
                                alert = 2
                            elif conf['ps_watt_max_warn'] >= tmp[key][stat_t] <= conf['ps_watt_min_warn']:
                                tmp[key][stat_t] = '%.0f(!)' %tmp[key][stat_t]
                                if alert != 2:
                                    alert = 1
                            else: tmp[key][stat_t] = '%.0f' %tmp[key][stat_t]
                        ...
                        tmp[key][stat_t] = float(tmp[key][stat_t])/10
                        if type(conf['sensor_temp_min_warn']) == int\
                            or type(conf['sensor_temp_max_warn']) == int\
                            or type(conf['sensor_temp_min_crit']) == int\
                            or type(conf['sensor_temp_max_crit']) == int:
                        ...
                        if max_crit >= tmp[key][stat_t] <= min_crit:
                            tmp[key][stat_t] = '%.1f(!!)' %tmp[key][stat_t]
                            alert = 2
                        elif max_warn >= tmp[key][stat_t] <= min_warn:
                            tmp[key][stat_t] = '%.1f(!)' %tmp[key][stat_t]
                            if alert != 2: alert = 1
                        else: tmp[key][stat_t] = '%.1f' %tmp[key][stat_t]
    ...
    def main(self):
        ...
        if self.hardware[2] == 'PDisk':
            ...
            if self.no_alert is None:
                hw_dict, exit_code = self.raise_alert(hw_dict, value_on_alert)
            tmp = '%s %s (%s) %.2f GB: %s, PowerStat: %s, HotSpare: %s [%s, S/N: %s]'
            for hw in hw_dict.values():
                for v in value_on_alert:
                    v = int(v)
                    hw[v] = hw[v].upper()
                output.append(tmp % (self.hardware[2], hw[0], hw[1].split()[-1].replace('"', ''), float(hw[5])/1024, hw[3], hw[7], hw[6].replace('HotSpare', '').replace('notASpare', 'no'), hw[2].replace('"', ''), hw[4].replace('"', '')))
            ...
            for hw in hw_dict.values():
                for v in value_on_alert:
                    ...
                    hw[v] = hw[v].upper()
                if self.perf is True:
                    perf_data = ' | Temperature=%sC;%s;%s;;' % (hw[3].split('(')[0], conf['sensor_temp_max_warn'], conf['sensor_temp_max_crit'])
                else:
                    perf_data = ''
                if self.no_alert is None:
                    output.append(tmp % (hw[4].replace('"', ''), hw[3], hw[1], hw[2], perf_data))
                else:
                    output.append(tmp % (hw[4].replace('"', ''), float(hw[3])/10, hw[1], hw[2], perf_data))
            ...
            if self.no_alert is None:
                hw_dict, exit_code = self.raise_alert(hw_dict, value_on_alert)
                tmp = '%s %s: %s, Volt I/O: %s V/%s V, Current: %s A, Watt I/O: %.0f W/%s W%s'
            else:
                tmp = '%s %s: %s, Volt I/O: %s V/%.0f V, Current: %.1f A, Watt I/O: %.0f W/%.0f W'
            for hw in hw_dict.values():
                for v in value_on_alert:
                    v = int(v)
                    hw[v] = hw[v].upper()
                if self.perf is True:
                    perf_data = ' | VoltINPUT=%sV;;;; VoltOUTPUT=%sV;;;; Current=%sA;;;; WattINPUT=%.0fW;;;; WattOUTPUT=%sW;;;;' %(hw[3], hw[6].split('(')[0], hw[5].split('(')[0], float(hw[4])/10, hw[2].split('(')[0])
                else:
                    perf_data = ''
                if self.no_alert is None:
                    output.append(tmp % (self.hardware[2], hw[0], hw[1], hw[3], hw[6], hw[5], float(hw[4])/10, hw[2], perf_data))
                else:
                    output.append(tmp % (self.hardware[2], hw[0], hw[1], hw[3], float(hw[6])/1000, float(hw[5])/10, float(hw[4])/10, float(hw[2])/10))
        elif self.hardware[2] == 'Memory':
            value_on_alert = [1, 2]
            if self.no_alert is None:
                hw_dict, exit_code = self.raise_alert(hw_dict, value_on_alert)
            #--debug print hw_dict
            tmp = '%s %s (%s) %.2f GB/%s MHz: %s/%s [%s, %s, S/N: %s]'
            for hw in hw_dict.values():
                for v in value_on_alert:
                    v = int(v)
                    hw[v] = hw[v].upper()
                output.append(tmp % (self.hardware[2], hw[0], hw[4].replace('.', ' ').replace('"', ''), float(hw[5].split()[-1])/(1024*1024), hw[6].split()[-1], hw[1], hw[2], hw[3].replace('deviceTypeIs', ''), hw[7].replace('"', ''), hw[8].replace('"', ''),))
            ...
            if self.no_alert is None:
                hw_dict, exit_code = self.raise_alert(hw_dict, value_on_alert)
            tmp = '%s %s (%s): %s/%s, %s (%.2f GB), BadBlock: %s [%s]'
            for hw in hw_dict.values():
                for v in value_on_alert:
                    v = int(v)
                    hw[v] = hw[v].upper()
                output.append(tmp % (self.hardware[2], hw[0], hw[1].replace('"', ''), hw[5], hw[2], hw[4].replace('r', 'RAID-'), float(hw[3])/1024, hw[6], hw[7].replace('"', '')))
